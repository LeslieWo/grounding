"""The LangGraph for grounding companionship -- this is a genuine agent loop.

This is exactly what sets it apart from a "workflow": no more hard-coded fixed
sequence. Each turn, the LLM itself **reads your state and decides what to do
next** (keep asking / switch to another photo / time for a gentle wind-down),
then picks a direction that fits the moment to ask about. The human only
replies; the agent holds the steering wheel.

One turn = one GRAPH invocation:
    decide (LLM senses emotion + picks an action)
       --conditional edge: switch photo--> switch (pick another) --> compose (LLM speaks)
       --conditional edge: continue/wind down--> compose (LLM speaks)

The opening (first turn) is the exception: gently receive the distress and
bring the first photo forward right away. State lives in the UI's (app.py)
session, so this graph is "pure function"-style -- stable, refresh-proof.
"""
import os
import re
import random
...
# The decision brain / photo-picker brain use a fast small model (these are
# internal judgments -- no need for 32b warmth, just speed); only the words
# actually spoken (compose) use the big model from .env. Configurable via the
# FAST_MODEL environment variable.
FAST_MODEL = os.environ.get("FAST_MODEL", "qwen2.5vl:7b")

# A menu of directions for the "decision brain" to consult (not a fixed
# sequence -- the agent may pick from it or go beyond it)
FOCUS_MENU = "看见的画面 / 声音 / 触感与温度 / 气味或味道 / 所在的位置 / 天气 / 和谁在一起、在做什么 / 此刻的心情"


# ---------- The agent's "decision brain" output (structured) ----------
...
# ---------- The agent's "photo-picker brain": read how they feel, choose the
# best-fitting photo from the memory bank ----------
...
def _photo_menu(bank: List[dict]) -> str:
    rows = []
    for m in bank:
        # The scene (see/where) is the reliable signal, so it goes first;
        # emotion labels are unreliable, so downplay them
...
def pick_memory(bank: List[dict], feeling_text: str, exclude_ids=None) -> dict:
    """Have the LLM read how they feel right now and pick the best-fitting photo
    from the bank. Falls back to random when picking fails."""
...
# For "non-question" speech (recap / asking whether to end / farewell / crisis)
# -- drops the "ask about the photo" ruleset so the two don't fight each other
...
class GState(TypedDict, total=False):
    bank: List[dict]          # All memories (used when switching photos)
    memory: dict              # The memory card for the current photo
    shown_ids: List[str]      # Photo ids already shown this session
    history: List[dict]       # [{role: me/companion, text}]
    covered: List[str]        # Directions already gently talked through
    last_user: str
    turn: int
    arm: str                  # A/B arm: 'random' (dumb baseline) or 'agent' (smart
                              # picking) -- one arm locked for the whole session
    avoid: List[dict]         # Lines the user 👎'd + reasons, used as negative
                              # examples before speaking (instant RLHF)
    contact: dict             # Trusted contact (personal data, never stored server-side;
                              # the client sends it along with each request)
    # ---- Outputs ----
    action: str
    focus: str
    tool_name: str
    tool_reason: str
    tool_result: dict          # Result after run_tool executes a tool (e.g. crisis contact)
    emotional_read: str
    reasoning: str
    pick_reason: str
...
def _is_unknown(v: str) -> bool:
    """Treat "unknown / to be filled in" fields as absent -- never feed them to
    the companion. Handles both full-width and half-width parentheses, and even
    if the model tacked its own speculation on afterwards (e.g. "...someone
    important may have come along"), discard the whole thing, so baseless
    guesses never enter the conversation as facts."""
    v = (v or "").strip()
    if not v:
        return True
...
def _tidy(text: str, single_question: bool = True) -> str:
    """Tidy up the companion's words: strip parenthetical asides / asterisk
    actions; cut at the first question so there's only one, and it stays short."""
    t = (text or "").strip()
    t = re.sub(r"[（(][^）)]*[）)]", "", t)   # Strip asides like "(gently puts down a hand)"
    t = re.sub(r"\*[^*]*\*", "", t)          # Strip things like *softly*
    if single_question:
        m = re.search(r"[？?]", t)
        if m:
            t = t[: m.end()]
...
def _history_text(history: List[dict], limit=None) -> str:
    """By default give the model the **entire** conversation of this session
    (sessions are short enough to stay coherent).
    limit is only a safety valve for extreme lengths."""
    if not history:
        return "（还没开始）"
...
def _emotion_trail(history: List[dict]) -> str:
    """String together the emotion the agent read each turn into one trail,
    used to judge "recovery" and to write the recap."""
...
# ---------- Node 1: decision brain (this is where the agent "calls the shots") ----------
def decide(state: GState) -> dict:
    ...
    if is_opening:
        # Even at the opening, **actually read** what they said -- never hard-code it
...
现在请决定下一步。"""
    ...
    # Use one model for the whole conversation (GROUNDING_MODEL from .env), to
    # avoid loading/unloading two models back and forth every turn (thrashing)
    ...
    # The opening only allows ask or (when there's danger) use_tool -- never
    # switch/close on the very first line
    action = d.action
...
# ---------- Node 2: switch photos (only taken when the agent decides to switch;
# the "photo-picker brain" chooses -- no longer random) ----------
def switch(state: GState) -> dict:
    ...
    if state.get("arm") == "random":
        # Dumb baseline: shove in a random one (A/B control)
        ...
        # Smart arm: read their state right now and let the LLM pick a better fit
...
# ---------- Node: execute a tool (only taken when the agent decides use_tool) ----------
def run_tool(state: GState) -> dict:
    """Look up the tool registry, execute, write the result back into state.
    Then hand off to compose, which presents the result while continuing to
    keep them company."""
...
# ---------- Node 3: speak (turn the decision into a gentle piece of speech) ----------
...
{_history_text(state.get('history', []))}"""
    ...
    # Instant RLHF: lines they 👎'd before + reasons, used as negative examples
...
这一轮想引导的方向（内部参考，**别照抄字面**，用你自己的话问）：{focus or '照片里一个还没聊到的、能让 ta 自己开口的温柔细节'}"""
    ...
    # Find the companion's previous line -- explicitly tell the model not to
    # parrot it (7b loves copying its own last line)
    prev_companion = ""
    ...
    # Only the two "question-asking" actions get trimmed to a single question
    # mark; recap/ending/farewell/crisis keep the full text
    single_q = action in ("ask", "switch_photo")
    # Non-question speech uses the lightweight system prompt, so it doesn't
    # fight the "ask about the photo" rules and leak instructions
    ...
    msg = _generate(0.6)
    # Programmatic backstop: if it's too similar to the previous line, retry
    # once with higher temperature plus a hard instruction, guaranteeing no repeats
...
# ---------- Assembling the graph: decide --conditional--> (switch / run_tool) --> compose --> END ----------
...
def next_turn(bank, memory, shown_ids, history, last_user, turn, covered, arm="agent", avoid=None,
              contact=None) -> dict:
    """Run one turn of the agent loop. Returns the next piece of companion speech
    plus the agent's decision this turn (including whether it switched photos and
    whether it wants to wind down).
    arm controls the photo-switching strategy (random = dumb baseline / agent =
    smart picking); one arm is locked for the whole session.
    avoid holds lines the user 👎'd + reasons, used as negative examples before
    speaking (instant RLHF).
    contact is the trusted contact (sent by the client; in local dev, if absent
    it falls back to the local config.json)."""
    out = GRAPH.invoke({
        "bank": list(bank or []),
        "memory": memory or {},
